...
/*< init 2D linear interpolation >*/
{
    ...
    for (ia=0; ia<na; ia++) {
		...
		if(aa[ia].z >= fdm->ozpad && 
			...
			ca->jz[ia] = floor((aa[ia].z-fdm->ozpad)/fdm->dz);
			ca->jx[ia] = floor((aa[ia].x-fdm->oxpad)/fdm->dx);
...
/*------------------------------------------------------------*/
void lint2d_inject_fx(float *uu,
					  float *ww,
					  float *rox,
					  int    nc,
					  int    it,
...
/*< inject vertical force into wavefield >*/
{
    int   ia;
    float wa, scal;
...
#pragma omp for \
	...
	for (ia=0; ia<ca->n; ia++) {
		...
		wa = ww[ia + 1*ca->n + it*ca->n*nc];
		
		scal = rox[(ca->jx[ia]+0)*fdm->nzpad+ca->jz[ia]+0] * fdm->dx;
		uu[(ca->jx[ia]+0)*fdm->nzpad+ca->jz[ia]+0]   += wa * scal * ca->w00[ia]; /* IMPORTANT - THE SOURCE IS ADDED (NOT SUBTRACTED LIKE IN PAUL SAVA'S CODE */
		
		scal = rox[(ca->jx[ia]+0)*fdm->nzpad+ca->jz[ia]+1] * fdm->dx;
		uu[(ca->jx[ia]+0)*fdm->nzpad+(ca->jz[ia]+1)] += wa * scal * ca->w01[ia];
		
		scal = rox[(ca->jx[ia]+1)*fdm->nzpad+ca->jz[ia]+0] * fdm->dx;
		uu[(ca->jx[ia]+1)*fdm->nzpad+ca->jz[ia]+0]	+= wa * scal * ca->w10[ia];
		
		scal = rox[(ca->jx[ia]+1)*fdm->nzpad+ca->jz[ia]+1] * fdm->dx;
		uu[(ca->jx[ia]+1)*fdm->nzpad+(ca->jz[ia]+1)]+= wa * scal * ca->w11[ia];
    }
}

/*------------------------------------------------------------*/
void lint2d_inject_fz(float *uu,
					  float *ww,
					  float *rox,
					  int    nc,
					  int    it,
...
/*< inject horizontal force into wavefield >*/
{
    int   ia;
    float wa, scal;
...
/*< inject pressure source into wavefield >*/
{
    int   ia;
    float wa, scal;
...
#pragma omp for \
	...
	for (ia=0; ia<ca->n; ia++) {
		...
		wa = ww[ia + 0*ca->n + it*ca->n*nc];
		
		scal = vel2ro[(ca->jx[ia]+0)*fdm->nzpad+ca->jz[ia]+0] * fdm->dt;
		uu[(ca->jx[ia]+0)*fdm->nzpad+ca->jz[ia]+0]   += wa * scal * ca->w00[ia]; /* IMPORTANT - THE SOURCE IS ADDED (NOT SUBTRACTED LIKE IN PAUL SAVA'S CODE */
		
		scal = vel2ro[(ca->jx[ia]+0)*fdm->nzpad+ca->jz[ia]+1] * fdm->dt;
		uu[(ca->jx[ia]+0)*fdm->nzpad+(ca->jz[ia]+1)] += wa * scal * ca->w01[ia];
		
		scal = vel2ro[(ca->jx[ia]+1)*fdm->nzpad+ca->jz[ia]+0] * fdm->dt;
		uu[(ca->jx[ia]+1)*fdm->nzpad+ca->jz[ia]+0]	+= wa * scal * ca->w10[ia];
		
		scal = vel2ro[(ca->jx[ia]+1)*fdm->nzpad+ca->jz[ia]+1] * fdm->dt;
		uu[(ca->jx[ia]+1)*fdm->nzpad+(ca->jz[ia]+1)]+= wa * scal * ca->w11[ia];
    }
}
...
/*< extract from wavefield >*/
{
    ...
    for (ia=0; ia<ca->n; ia++) {
		dd[ia] =
	    	uu[ca->jx[ia]*fdm->nzpad+ca->jz[ia]        ] * ca->w00[ia] +
...
/*------------------------------------------------------------*/
void extract_velx_interp(float *ve,
					float *vex_abc,
					lint2d cab,
					fdm2d fdm)
...
/*< extract from wavefield >*/
{
    ...
    for (k=0; k<cab->n; k++) {
		...
		float c1=9.0/16.0, c2=9.0/16.0;
		float c3=-1.0/16.0, c4=-1.0/16.0;


		vex_abc[k] = c1 * ve[(ix-1)*fdm->nzpad+iz] +
					c2 * ve[ix * fdm->nzpad+ iz] +
					c3 * ve[(ix-2) * fdm->nzpad + iz] +
					c4 * ve[(ix+1) * fdm->nzpad + iz];
    }
}  

/*------------------------------------------------------------*/
void extract_velz_interp(float *ve,
					float *vez_abc,
					lint2d cab,
					fdm2d fdm)
...
/*< extract from wavefield >*/
{
    ...
    for (k=0; k<cab->n; k++) {
		...
		vez_abc[k] = c1 * ve[ix*fdm->nzpad+(iz-1)] +
					c2 * ve[ix * fdm->nzpad+ iz] +
					c3 * ve[ix * fdm->nzpad + (iz-2)] +
					c4 * ve[ix * fdm->nzpad + (iz+1)];
    }
}  
...
/*------------------------------------------------------------*/
void huygens_dip_apply(float *vz,
					   float *vx,
					   float *p,
					   float *roz,
					   float *rox,
					  ...
					  
{
	...
	{
		/*Bottom boundary*/
		for (ix=ini+1; ix<end; ix++) { 
			iz = end;

			scal = roz[ix*fdm->nzpad+iz] * fdm->dz;
			vz[ix*fdm->nzpad+iz] -= scal* (w1*(p[ix*fdm->nzpad+(iz - 1)] + p[ix*fdm->nzpad+(iz)]) +
										    w2*(p[ix*fdm->nzpad+(iz - 2)] + p[ix*fdm->nzpad+(iz + 1)]));
		}
		...
		/*Right boundary*/
		for (iz=ini+1; iz<end; iz++) { 
			ix = end;
		
			scal = rox[ix*fdm->nzpad+iz] * fdm->dx;
			vx[ix*fdm->nzpad+iz] -= scal* (w1*(p[(ix - 1)*fdm->nzpad+(iz)] + p[(ix)*fdm->nzpad+iz]) +
										   w2*(p[(ix - 2)*fdm->nzpad+(iz)] + p[(ix + 1)*fdm->nzpad+iz]));
		}
		...
		// vx[ix*fdm->nzpad+iz] -= scal* (-0.707*(w1*(p[(ix - 1)*fdm->nzpad+iz] + p[ix*fdm->nzpad+iz]) +
		// 									   w2*(p[(ix - 2)*fdm->nzpad+iz] + p[(ix + 1)*fdm->nzpad+iz])));
		
		// vz[ix*fdm->nzpad+iz] -= scal* (0.707*(w1*(p[ix*fdm->nzpad+(iz - 1)] + p[ix*fdm->nzpad+(iz)]) +
		// 									  w2*(p[ix*fdm->nzpad+(iz - 2)] + p[ix*fdm->nzpad+(iz + 1)])));
...
/*------------------------------------------------------------*/
void huygens_mon_apply(float *p,
					   float *vz,
					   float *vx,
					   float *vel2ro,
					   int	  ini,
					...
					
{
	...
	#endif 
	{
		...
		for (ix=ini+1; ix<end; ix++) {
			...
			scal = vel2ro[ix*fdm->nzpad+iz] * fdm->dt;
			p[ix*fdm->nzpad+iz] -= scal * (w1 * (vz[ix*fdm->nzpad+(iz)]     + vz[ix*fdm->nzpad+(iz + 1)]) +
										    w2 * (vz[ix*fdm->nzpad+(iz - 1)] + vz[ix*fdm->nzpad+(iz + 2)]));
		}
		...
		for (iz=ini+1; iz<end; iz++) {
			...
			scal = vel2ro[ix*fdm->nzpad+iz] * fdm->dt;
			p[ix*fdm->nzpad+iz] -= scal * (w1 * (vx[(ix)*fdm->nzpad+iz]     + vx[(ix + 1)*fdm->nzpad+iz]) + 
										   w2 * (vx[(ix - 1)*fdm->nzpad+iz] + vx[(ix + 2)*fdm->nzpad+iz]));	  
		}
		...
		// p[ix*fdm->nzpad+iz] -= scal* (-0.707*(w1*(vx[(ix)*fdm->nzpad+iz]     + vx[(ix + 1)*fdm->nzpad+iz]) +
		// 									  w2*(vx[(ix - 1)*fdm->nzpad+iz] + vx[(ix + 2)*fdm->nzpad+iz])) +
		// 							   0.707*(w1*(vz[ix*fdm->nzpad+(iz)]     + vz[ix*fdm->nzpad+(iz + 1)]) +
		// 									  w2*(vz[ix*fdm->nzpad+(iz - 1)] + vz[ix*fdm->nzpad+(iz + 2)])));
